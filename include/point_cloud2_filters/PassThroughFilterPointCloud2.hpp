...
bool PassThroughFilterPointCloud2::configure()
{
    ...
    filters::FilterBase<sensor_msgs::msg::PointCloud2>::getParam(std::string("filter_field_name"), filter_field_name_);
    RCLCPP_INFO(logging_interface_->get_logger(),"[%s] Using field name='%s'", getName().c_str(), filter_field_name_.c_str());
    
    filters::FilterBase<sensor_msgs::msg::PointCloud2>::getParam(std::string("filter_limit_min"), filter_limit_min_);
    RCLCPP_INFO(logging_interface_->get_logger(),"[%s] Using limit min='%f'", getName().c_str(), filter_limit_min_);
    
    filters::FilterBase<sensor_msgs::msg::PointCloud2>::getParam(std::string("filter_limit_max"), filter_limit_max_);
    RCLCPP_INFO(logging_interface_->get_logger(),"[%s] Using limit max='%f'", getName().c_str(), filter_limit_max_);
